...
public class GridManagement : MonoBehaviour
{
    ...
    public PiecePrefab[] piecesPrefabs;
    ...
    void Awake()
    {
        ...
        for (int x = 0; x < xDim; x++)
        {
            ...
            {
                GameObject background = GameObject.Instantiate(backgroudPrefab, GetWorldPosition(x,y), Quaternion.identity);
        ...
        {
            if (!piecePrefabDict.ContainsKey(piecesPrefabs[i].type))
            {
                piecePrefabDict.Add(piecesPrefabs[i].type, piecesPrefabs[i].prefab);
            }
        }
    ...
    {
        return new Vector2(transform.position.x - xDim * 0.5f + x + 0.5f, transform.position.y + yDim * 0.5f - y - 0.5f);
    }
    ...
    public void SwapPieces(GamePiece piece1, GamePiece piece2)
    {
        ...
        if (piece1.IsMovable() && piece2.IsMovable())
        {
                ...
                || piece1.Type == PieceType.RAINBOW || piece2.Type == PieceType.RAINBOW)
            {
                ...
                if(piece1.Type == PieceType.RAINBOW && piece2.IsClearable())
                {
                ...
                if (piece2.Type == PieceType.RAINBOW && piece1.IsClearable())
                {
...
//                level.OnMove();
            ...
            {
                pieces[piece1.X, piece2.Y] = piece1;
                ...
                if (pieces[x, y].IsClearable())
                {
                    ...
                    if (match != null)
                    {
                        ...
                        if (specialPieceType != PieceType.COUNT)
                        {
                            ...
                            Debug.Log(specialPieceX + " , " + specialPieceY);

                            if((specialPieceType == PieceType.COLUMN_CLEAR||specialPieceType==PieceType.ROW_CLEAR)
    ...
    public void ClearObstacles(int x, int y)
    {
        for( int adjacentX = x-1; adjacentX <= x+1; adjacentX++)
        {
            ...
            {
                if(pieces[adjacentX, y].Type == PieceType.ROCK && pieces[adjacentX, y].IsClearable())
                {
        ...
        {
            if (adjacentY != y && adjacentY >= 0 && adjacentY < xDim)
            {
                if (pieces[x, adjacentY].Type == PieceType.ROCK && pieces[x, adjacentY].IsClearable())
                {
    ...
    public void ClearColurm(int column)
    {
        for(int y =0; y<yDim; y++)
        {
            ClearPiece(column, y);
